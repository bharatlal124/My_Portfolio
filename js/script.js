...
window.onscroll = () => {
  sections.forEach((sec) => {
    ...
    if (top >= offset && top < offset + height) {
      ...
      // Play animation only the first time
      if (!sec.classList.contains("animation-played")) {
        sec.classList.add("show-animate", "animation-played");
        // The `animation-played` class prevents re-triggering
      }
    }
  });
